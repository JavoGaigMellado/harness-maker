...
# Pieza · Instrucciones persistentes
...
1. Resuelve el estado objetivo: usa la ruta indicada por la persona; si no la hay, lee
   ...
   adopta por ser el único que aparece. Localiza `prompts` en su ruta y explica la `regla` y
...
Las instrucciones que están siempre puestas.
...
Pregunta principal: **¿Qué instrucciones están siempre puestas?**

En el ejemplo del ayudante de correo: Está escrito que respondes en usted, que nunca prometes fechas sin confirmar y que si el cliente pregunta por precios, no contestas: avisas.

## Puntos clave

- Cuatro mecanismos, cuatro usos. **Instrucciones** para lo permanente. **Reglas** para lo situacional. **Skills o comandos** para lo que se invoca. **Hooks** para lo que no puede fallar.
...
1. Escribir la orientación permanente en un solo sitio.
2. Marcar qué es preferencia personal y qué es regla del proyecto.
3. Definir la prioridad cuando dos instrucciones chocan.
4. Revisar periódicamente qué sigue vigente.
5. Lo que no puede fallar, ponerlo donde el sistema lo imponga.
...
`<directorio-del-estado>/piezas/prompts.md` e incluye al final un bloque cercado
