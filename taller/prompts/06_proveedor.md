...
1. Resuelve el estado objetivo: usa la ruta indicada por la persona; si no la hay, lee
   ...
   adopta por ser el único que aparece. Localiza `proveedor` en su ruta y explica la `regla` y
...
Sobre qué trabajas y con qué capacidad.
...
Pregunta principal: **¿Sobre qué plataforma trabajas, con qué modelo y a qué coste?**

En el ejemplo del ayudante de correo: Usas Claude Code con el modelo por defecto. Sabes cuánto gastas al mes. Si el servicio se cae, respondes los correos urgentes a mano y el resto espera.
...
- **Solo si tienes código propio:** aislar la llamada al proveedor en un sitio. Es condicional, no obligatorio.
...
1. Escribir qué plataforma y qué modelo usas, y por qué.
2. Anotar coste y límites previstos.
3. Revisar condiciones de datos y privacidad.
4. Definir qué se hace si el servicio no está.
5. Anotar qué parte es portable.
6. Si hay código propio: aislar la llamada en un único sitio.
...
`<directorio-del-estado>/piezas/proveedor.md` e incluye al final un bloque cercado
