...
1. Resuelve el estado objetivo: usa la ruta indicada por la persona; si no la hay, lee
   ...
   adopta por ser el único que aparece. Localiza `conocimiento` en su ruta y explica la `regla` y
...
Pregunta principal: **¿De dónde saca lo que afirma?**

En el ejemplo del ayudante de correo: Las fuentes buenas son la ficha del proyecto y el histórico de ese cliente. Si no coinciden, manda la ficha. Si no hay dato, el borrador dice «pendiente de confirmar» en vez de inventarlo.

## Puntos clave

- Antes de elegir técnica, responde lo básico: qué vale, quién lo mantiene, cuál gana.
- Separa lo que se puede usar de lo que **no puede salir de su sitio**.
- Sin procedencia no hay forma de comprobar nada. Decide cómo se cita.
- El caso más olvidado: qué pasa si no hay fuente. **«No lo sé» tiene que valer.**
...
1. Listar las fuentes autorizadas y quién mantiene cada una.
2. Escribir cuál manda en caso de conflicto.
3. Marcar qué información no puede salir de su origen.
4. Definir cómo se cita la procedencia.
5. Decidir qué se responde cuando no hay fuente.
...
`<directorio-del-estado>/piezas/conocimiento.md` e incluye al final un bloque cercado
