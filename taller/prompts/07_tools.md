...
# Pieza · Herramientas e integraciones
...
1. Resuelve el estado objetivo: usa la ruta indicada por la persona; si no la hay, lee
   ...
   adopta por ser el único que aparece. Localiza `tools` en su ruta y explica la `regla` y
...
Con qué puede actuar, además de hablar.
...
Pregunta principal: **¿Con qué puede actuar, y qué puede hacer sin preguntarte?**

En el ejemplo del ayudante de correo: Puede leer la carpeta del proyecto y buscar en la web sin preguntar. Puede escribir en tu carpeta de borradores avisando. Para tocar el sistema del cliente, pide permiso cada vez.

## Puntos clave

- Leer es barato y casi siempre compensa.
- Escribir aporta mucho **cuando está acotada, autorizada, registrada y es reversible**.
...
1. Listar qué herramientas hacen falta y para qué tarea.
2. Separar las que leen de las que escriben o gastan.
3. Dar identidad y permiso mínimo a cada una.
4. Escribir cuáles piden confirmación.
5. Anotar qué pasa si una falla a medias.
6. Dejar por escrito la alternativa manual.
...
`<directorio-del-estado>/piezas/tools.md` e incluye al final un bloque cercado
