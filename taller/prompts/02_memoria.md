...
1. Resuelve el estado objetivo: usa la ruta indicada por la persona; si no la hay, lee
   ...
   adopta por ser el único que aparece. Localiza `memoria` en su ruta y explica la `regla` y
...
Qué se guarda de una vez para la siguiente.
...
Pregunta principal: **¿Qué se guarda de una vez para la siguiente?**

En el ejemplo del ayudante de correo: Recuerda que firmas «Un saludo» (memoria). Sabe que ese correo está pendiente de la respuesta del cliente (estado). Consulta la ficha (conocimiento). Aplica tu estilo (instrucciones). Guarda lo que hizo el martes (historial).
...
1. Decidir qué se guarda de cada uno de los cinco tipos.
2. Elegir dónde vive cada tipo y quién lo edita.
3. Definir qué se escribe al cerrar una sesión.
4. Fijar cada cuánto se revisa y qué se borra.
5. Comprobar que otra persona entiende el estado solo leyendo.
...
`<directorio-del-estado>/piezas/memoria.md` e incluye al final un bloque cercado
